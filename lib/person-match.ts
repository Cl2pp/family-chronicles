 ...
 * stores displayName "Clemens" + familyName "Ortlepp". A name only matches when it
...
export function findPersonByName<T extends MatchablePerson>(
  ...
  const exact = people.filter((p) => norm(p.displayName) === wanted);
  ...
  const candidates = people.filter((p) => {
    ...
    // "Ava" → "Ava Naoko" (given name / prefix of the stored name)
    ...
    // "Clemens Ortlepp" → displayName "Clemens" + familyName "Ortlepp"
