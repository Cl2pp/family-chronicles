...
export interface ToolContext {
  ...
  /**
   * Called by the agent loop on a timer while each model call is in flight. Lets the
   * caller refresh its reply-generation claim so a long turn — even one slow step —
   * is never mistaken for a crashed one (and double-generated by a recovery sync).
   * Must not throw.
   */
  keepAlive?: () => void;
  /**
