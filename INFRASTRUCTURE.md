...
| `BOOK_ORDERING_ALLOWED_EMAILS` | Comma-separated account emails allowed to place **real Gelato orders in the app** (`lib/book-orders.ts`); everyone else keeps the email-us flow | Unset = nobody. Set on **both** web and worker (the worker only needs it for consistency; the gate runs in web) |
| `GELATO_ORDER_TYPE` | `draft` (default) or `order` | `draft` creates a reviewable draft in the Gelato dashboard — nothing is printed or charged until you convert it there. Set `order` only once the first draft has been checked. Set on the **worker** (that's who submits) |
