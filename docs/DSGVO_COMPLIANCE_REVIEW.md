...
| 2 | C3: **✅ implemented** (signup consent checkbox gating email and Google signup — incl. `disableImplicitSignUp` so a Google click on /login can't create an account — plus legal links on auth pages). Still open: consent is not yet *evidenced* server-side (Art. 7 Abs. 1) — store a consented-at timestamp + policy version on the user at signup; fold into the C2 account work | S |
