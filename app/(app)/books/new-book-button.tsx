...
import { IconBook2, IconCake, IconPhoto, IconPlus } from '@tabler/icons-react';
import { notifications } from '@mantine/notifications';
import { useI18n } from '@/lib/i18n/client';
import { createBirthdayBookAction, createBookAction, createPhotoBookAction } from './actions';

/**
 * "New book" — offers the fixed Birthday recipe alongside the general story/photo
 * starting points, then creates a draft and opens its builder. Story-based choices start
 * from every ready story; a photo book starts empty and is filled by the bulk uploader.
 */
export function NewBookButton({ label }: { label: string }) {
  ...
  function createBirthdayBook() {
    startTransition(async () => {
      const result = await createBirthdayBookAction();
      if (result?.error) notifications.show({ message: result.error, color: 'red' });
    });
  }
      ...
      <Modal opened={opened} onClose={() => setOpened(false)} title={label} centered>
        <Stack gap="sm">
          ...
          <Card
            ...
            onClick={createBirthdayBook}
          >
            <Group gap="sm" wrap="nowrap">
              <IconCake size={24} stroke={1.6} color="var(--mantine-color-brand-6)" />
              <Stack gap={2}>
                <Text fw={600}>{t.books.kindBirthday}</Text>
                <Text fz={13} c="dimmed">
                  {t.books.kindBirthdayHint}
                </Text>
              </Stack>
            </Group>
          </Card>
